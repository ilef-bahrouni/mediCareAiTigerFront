...
role ?: string ; 
 private injector = inject(Injector);
  ...
  this.doctorService.getAllAppointment(objList).subscribe({
      next: (res: any) => {
        this.loading = false;
        if (res.code === 200) {
...
delete(item: any) {
  ...
  modalRef.result
    .then((res: any) => {
      // if (res === 'confirm') {  
        this.doctorService.deleteAppointment(item.id).subscribe({
          next: (res: any) => {
            if (res.code === 200) { 
              this.toastr.showSuccess('APPOINTMENT.DELETED_SUCCESS');
              this.initData();
            } else {
              this.toastr.showError('APPOINTMENT.DELETED_ERROR');
            } 
          },
          error: (err : any) => {
            this.toastr.showError('ERRORSERVENU');
